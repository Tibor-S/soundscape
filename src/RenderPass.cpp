...
RenderPass::RenderPass(Spec &spec) : DeviceParent(spec.device) {
...
VkRenderPass RenderPass::create_render_pass(Device* device, VkFormat color_format, VkFormat depth_format) {
    ...
    depthAttachment.samples = device->max_sample_count();
    ...
    if (vkCreateRenderPass(device->logical_device_handle(), &renderPassInfo, nullptr, &render_pass_handle) != VK_SUCCESS) {
