...
void main() {
    ...
    vec3 base_color = fragColor;
    if (fragTexCoord.x > -0.5 && fragTexCoord.x < 1.5 && fragTexCoord.y > -0.5 && fragTexCoord.y < 1.5) {
        vec3 base_color = texture(texSampler, fragTexCoord);
    }
    ...
    vec3 color = texture(texSampler, fragTexCoord) * light;
